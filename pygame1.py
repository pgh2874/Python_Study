...
#배경 이미지 불러오기
background = pygame.image.load("C:\\Python\\practice\Basic\\pygame_Basic\\Background.png")

# 캐릭터(스프라이트) 불러오기
character = pygame.image.load("C:\\Python\\practice\Basic\\pygame_Basic\\character.png")
...
#적  enemy character
enemy = pygame.image.load("C:\\Python\\practice\Basic\\pygame_Basic\\enemy.png")
...
#시작 시간 
start_ticks = pygame.time.get_ticks() #시간 tick받아옴
...
#20 fps : 1초 동안에 20번 동작 -> 1번 동작에 5만큼! 5*20 = 100
    ...
    character_X_pos += to_x * dt #프레임 값을 보정 => 프레임수 상관없이 속도 유지
    ...
    screen.blit(background, (0,0))# 배경그리기 # 0,0 => 맨왼쪽 위 기준
    ...
    # 타이머 집어넣기
    # 경과 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 #밀리세컨드(ms)를 초 단위로 표시
